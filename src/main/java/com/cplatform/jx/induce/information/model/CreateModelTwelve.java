 ...
 * @version 1.0.0
 */
public class CreateModelTwelve extends AbstractProcessThread{

	/** 日志记录器 */
	private Logger logger = Logger.getLogger(getClass());
	
	private Map <String,List<Integer>> modelTwelve = new HashMap<String, List<Integer>>();
	public CreateModelTwelve(){
		init(1000*60*2);
//		init(1000);
	}
	...
	protected void execTask() throws Exception {
		...
		lis = ServiceDb.getModelTwelveConfigs();
		...
		for(ModelTwelveConfigInfo info:lis){
			for(TDeviceInfo dInfo:devices){
				if(info.getDEVICE_CODE().equals(dInfo.getDEVICE_CODE())){
					...
					if(isEmergent==null||isEmergent==true){
						...
						//模板在播放紧急消息
						modelTwelve.remove(info.getDEVICE_CODE());
						break;
					}
					...
					if(allTime>0&&color>0){
						cont= ""+info.getROAD_NAME()+allTime/60+"分钟"+" ";	
						colorList.add(color);
					}
					else{
						break;
					...
					if(allTime>0&&color>0){
						cont= cont+info.getROAD_NAME2()+allTime/60+"分钟";	
						colorList.add(color);
					}
					else{
						break;
					...
					String fileName = info.getDEVICE_CODE() + "_" + dInfo.getDISPLAY_RESO().replace("*", "_") + "_12_"+time.substring(8, 14)+".png";
					if(!compare(colorList, oldList)){
						logger.debug("设备："+colorList +" newColor："+colorList+" 生成新消息"+cont);
...
//						DeviceItemInfo item = WriteSevenItem(cont,dInfo.getDISPLAY_RESO().trim(),dInfo.getDEVICE_CODE(),"1",col,info);
						DeviceItemInfo item = WriteTwelveItemWithImages(cont,dInfo.getDISPLAY_RESO().trim(),dInfo.getDEVICE_CODE(),fileName,Collections.max(colorList),info.getFONT_SIZE());
						if(item != null&&checkItemMap(dInfo.getDEVICE_CODE().trim(),item)){
							...
							items.add(item);
							logger.debug("模板12创建消息成功! equipment:"+dInfo.getDEVICE_CODE()+" 内容："+ cont);
						}
					}
					break;
				}
...
//        ServiceDb.SavePushItem(items);
	...
	private List<Integer> getOldColor(String provCode) {
		try{
		return modelTwelve.get(provCode)== null ? new ArrayList<Integer>():modelTwelve.get(provCode);
		}
		catch(Exception e){
			logger.error("遍历异常",e);
		}
	...
	private void putColor(String provCode,List<Integer> list){
		try{
			modelTwelve.remove(provCode);
			modelTwelve.put(provCode, list);
			}
			catch(Exception e){
				logger.error("更新异常",e);
			}
