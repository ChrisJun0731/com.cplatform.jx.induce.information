...
public class CreateModelTwelveA extends AbstractProcessThread {
...
//		init(1000*10);
	...
	protected void execTask() throws Exception {
		...
		lis = ServiceDb.getModelSixConfigs(12);
...
//					buf.append(info.getDEVICE_CODE() + "_" + dInfo.getDISPLAY_RESO().replace("*", "_") + "_6.png");
					...
					String fileName = info.getDEVICE_CODE() + "_" + dInfo.getDISPLAY_RESO().replace("*", "_") + "_12a_"+time.substring(8, 14)+".png";
						...
						if (!result.isEmpty()) {
								...
								if (ret) {
									...
									DeviceItemInfo item = WriteSixItem(dInfo.getDISPLAY_RESO().trim(), dInfo.getDEVICE_CODE(),
											filePath+fileName, "1",12,Collections.max(colorList));
...
//			ServiceDb.SavePushItem(items);
	...
	private boolean changeImage(String dealPath,String dealName, String path, List<ModelSixInfo> info,ModelSixConfigInfo confInfo) {
		...
		try {
			return ChangIMGColor.replaceImageColor(info, path, new Color(0xffffff), dealPath,dealName,confInfo,null);
		} catch (IOException e) {
